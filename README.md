...
- serverless : 0에서 무제한에 가까운 scaleout을 지원하며,  별도의 작업 없이 모니터링 가능한 서버를 즉시 릴리즈 할 수 있으며 다양한 장애상태를 스스로 극복한다. 제한된 인력상황에서 강력함.
- multi az : vpc,eni등을 수동설정하지 않는다면 기본적으로 multi-az로 높은 고가용성을 보장하며 별도의 비용이 발생하지 않는다.
...
- (지원예정) source generator 를 통한 coammand dispatch
## 기타
